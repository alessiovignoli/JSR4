...
def get_args():
    ...
    parser.add_argument("-p1", "--pos1", type=int, required=False, nargs='?', const=1, default=0, metavar="field/column position/index", help='default 0, integer pointing to the column/field to be used as a key in file1, WARNING the notation must be python based, meaning first field = 0')
    parser.add_argument("-p2", "--pos2", type=int, required=False, nargs='?', const=1, default=0, metavar="field/column position/index", help='same rule as above but this applies to file2 instead.')
    parser.add_argument("-d1", "--delimiter1", type=str, required=False, nargs='?', const=1, default='\t', metavar="delimiter/spatiator", help='default <tab>, with this flag the spatiator/delimiter of the file can be specified, it can be whatever string, for special charachters use either \' or \" to encapsulate them, they have to be pythonic simbols.')
    parser.add_argument("-d2", "--delimiter2", type=str, required=False, nargs='?', const=1, default='\t', metavar="delimiter/spatiator", help='all rules as above apply, it can be different from the value given to the above flag, in that case the output file will be delimited using the above flag value.')
    parser.add_argument("--check_types", type=bool, required=False, nargs='?', const=1, default=False, metavar="debug", help='default False, no type of the variables will be checked at line level inside the script, this flag is usefull for debugging.')
